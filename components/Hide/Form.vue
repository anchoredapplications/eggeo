<template>
    <main class="h-screen w-screen">
        <form class="flex flex-col gap-4 w-full pt-8" @submit.prevent="handleSubmit">
            ...
            <textarea v-model="description"  placeholder="Description" required class="p-2 border border-gray-300 rounded"></textarea>
            ...
            <button :disabled="isDisabled" type="submit" :class="{'bg-blue-100' : isDisabled}" class="p-2 bg-blue-500 text-white rounded hover:bg-blue-600">Submit</button>
...
<script setup>    
    ...
    const handleSubmit = async () => {
        ...
        if (!!trimTitle || !!trimDescription || !!trimUsername) {
            const response = await useFetch("/api/addEgg", { method: "post", body: {title: trimTitle, description: trimDescription, username: trimUsername, coords, color}})
            if (response) {
